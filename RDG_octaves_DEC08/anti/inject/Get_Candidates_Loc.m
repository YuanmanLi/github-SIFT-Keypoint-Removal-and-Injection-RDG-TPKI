...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [keypoints,candidate_Loc,G_layers] = Get_Candidates_Loc( I, par)
win = [3,3];                                                 %
zero_thr = par.zero_threshold;
keypoints = [];
candidate_Loc = cell(3,1);
%%%%%%%%%%parameter setting %%%%%%%%%%%%
f.nlevels = 3;
f.s_max = 4;
f.octave = 1;
...
for Loo = 1:loops
    ...
    Dog = {D1, D2, D3, D4, D5};
    %%%%%%%%%%%get the cadidates where Dog value is bigger than 3%%%%%%%%%%
    for i = par.cur_s :par.cur_s 
        tempD = Dog{i+1};
        temp_id = find(abs(tempD') >= par.minDog_inject);
        tempx = floor((temp_id-1)/b)+1;
        tempy = temp_id - (tempx-1)*b;
        loc   = [tempx, tempy, i*ones(length(temp_id),1)];
        candidate_Loc{i} = [candidate_Loc{i},loc];
    end
