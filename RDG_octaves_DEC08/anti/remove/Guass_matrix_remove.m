...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function P = Guass_matrix_remove(sigDiff, I)

[x]=2*max(ceil(4.0*sigDiff),1)+1;
Gauss_matrix = fspecial('gaussian', [x x], sigDiff);  %here we need to change
...
idx_pic_m = padarray(idx_pic_m, bounder, 'replicate');  %here need to change, no symemetric, no boundary.  
